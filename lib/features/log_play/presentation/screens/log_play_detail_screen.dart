...
class LogPlayDetailScreen extends ConsumerStatefulWidget {
  ...
  /// Constructor
  const LogPlayDetailScreen({super.key, required this.releaseId});

  @override
  ConsumerState<LogPlayDetailScreen> createState() =>
      _LogPlayDetailScreenState();
}
...
class _LogPlayDetailScreenState extends ConsumerState<LogPlayDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const CleoAppBar(title: 'Record Actions'),
      body: authState.when(
        data: (authData) {
          ...
          final styluses =
              authData.payload?.styluses.where((s) => s.active).toList() ?? [];
        ...
        error:
            (error, _) => Center(child: Text('Error loading record: $error')),
      ),
    );
  }
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(artistName, style: const TextStyle(fontSize: 16)),
  ...
  Widget _buildHistorySection(BuildContext context, Release release) {
    ...
    for (final play in release.playHistory) {
      print(
        'Play: ID=${play.id}, Date=${play.playedAt}, StylesID=${play.stylusId}',
      );
      allHistory.add(
        HistoryItem(
          type: 'play',
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        if (allHistory.isEmpty) const Text('No history recorded yet.'),
  ...
  Future<void> _logPlay(BuildContext context, Release release) async {
    ...
    try {
      ...
      final stylusId =
          _selectedStylusId != null ? int.parse(_selectedStylusId!) : null;
  ...
  Future<void> _logBoth(BuildContext context, Release release) async {
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Play and cleaning logged successfully'),
          ),
        );
  ...
  void _showEditDialog(BuildContext context, HistoryItem item) {
    ...
    final styluses =
        authState.value?.payload?.styluses.where((s) => s.active).toList() ??
        [];
      ...
      builder:
          (dialogContext) => StatefulBuilder(
            ...
            builder: (context, setState) {
              return AlertDialog(
                title: Text(
                  'Edit ${item.type == 'play' ? 'Play' : 'Cleaning'} Record',
                ),
                      ...
                      InkWell(
                        ...
                        child: Container(
                          ...
                          child: Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              Text(DateFormat('MM/dd/yyyy').format(editDate)),
                              const Icon(Icons.calendar_today),
                            ],
                          ),
                        ),
                    ...
                    onPressed: () {
                      ...
                      if (item.type == 'play') {
                        _updatePlay(
                          ...
                          editDate,
                          editStylusId != null
                              ? int.parse(editStylusId!)
                              : null,
                          editNotes,
                        );
                      } else {
                        _updateCleaning(context, item.id, editDate, editNotes);
                      }
                    },
    ...
    try {
      await ref
          ...
          .updatePlay(
            playId: playId,
            releaseId:
                widget.releaseId, // Include the release ID from the widget
            playedAt: playedAt,
            stylusId: stylusId,
            notes: notes,
          );
