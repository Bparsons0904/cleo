...
@riverpod
CleanlinessStatus recordCleanlinessStatus(Ref ref, int releaseId) {
  ...
  if (cleaningCountsAsync is AsyncLoading || playHistoryAsync is AsyncLoading) {
    return CleanlinessStatus.clean;
  }

  if (cleaningCountsAsync is AsyncError || playHistoryAsync is AsyncError) {
    return CleanlinessStatus.clean;
  }
  ...
  final hasBeenCleaned =
      cleaningCounts.containsKey(releaseId) && cleaningCounts[releaseId]! > 0;
  ...
  final recordPlays =
      playHistory.where((play) => play.releaseId == releaseId).toList();
