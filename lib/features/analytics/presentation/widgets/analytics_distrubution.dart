// lib/features/analytics/presentation/widgets/distribution_analysis.dart
...
class DistributionAnalysis extends ConsumerWidget {
          ...
          children: [
            ...
            Expanded(
              child: _buildDropdown<DistributionType>(
                ...
                items: DistributionType.values.map((type) => 
                  DropdownMenuItem(
                    value: type,
            ...
            Expanded(
              child: _buildDropdown<TopLimit>(
                value: topLimit,
                items: TopLimit.values.map((limit) => 
                  DropdownMenuItem(
                    value: limit,
                ...
                onChanged: (value) {
                  if (value != null) {
                    ref.read(distributionTopLimitProvider.notifier).state = value;
                  }
                },
              ),
            ),
  ...
  Map<String, int> _calculateDistribution(
    ...
    TopLimit limit,
    {bool countMode = true, required WidgetRef ref},
  ) {
    ...
    for (final play in playHistory) {
      ...
      switch (type) {
        case DistributionType.byGenre:
          key = release.genres.isNotEmpty 
              ? release.genres.first.name 
              : 'Uncategorized';
          break;
        
        case DistributionType.byArtist:
          key = release.artists.isNotEmpty && release.artists.first.artist != null
      ...
      if (countMode) {
        ...
        value = release.playDuration ?? 45; // Default to 45 minutes if not specified
      }
