...
class RecordDetailScreen extends ConsumerWidget {
  ...
  const RecordDetailScreen({super.key, required this.releaseId});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: CleoAppBar(title: release?.title ?? 'Record Details'),
      body: authState.when(
        data: (_) {
          if (release == null) {
              ...
              child: Column(
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      ...release.genres.map(
                        (genre) => _buildChip(context, genre.name),
                      ),
                      ...release.styles.map(
                        (style) =>
                            _buildChip(context, style.name, isStyle: true),
                      ),
                    ],
                  ),
                ],
  ...
  Widget _buildRecentPlays(
    ...
    final recentPlays =
        List<PlayHistory>.from(playHistory)
  ...
  //           child: const Row(
  //             mainAxisAlignment: MainAxisAlignment.spaceBetween,
  //             children: [Text('MP-110 (Primary)'), Icon(Icons.arrow_drop_down)],
  //           ),
  //         ),
  //         const SizedBox(height: 16),
  //
